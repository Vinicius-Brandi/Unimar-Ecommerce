...
def realizar_pagamento(collector_id, items, external_reference, fee_amount): # 'fee_amount' será sua comissão
    load_dotenv() 
    ...
    if not marketplace_access_token:
        raise Exception("MP_ACCESS_TOKEN (do marketplace) não encontrado nas variáveis de ambiente.")
    if not app_id_str:
        raise Exception("MP_APP_ID (do marketplace) não encontrado nas variáveis de ambiente.")
        
    sdk = mercadopago.SDK(marketplace_access_token) # SDK iniciado com o TOKEN DO SEU MARKETPLACE
