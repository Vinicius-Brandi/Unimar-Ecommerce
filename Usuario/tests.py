...
class CadastrarViewTest(TestCase):
    ...
    def test_cadastrar_post_usuario_existente(self):
        ...
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(
            str(messages_list[0]), "O username já está cadastrado, tente novamente!"
        )
    ...
    def test_cadastrar_post_sucesso(self):
        ...
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(
            str(messages_list[0]), "Cadastrado com sucesso! Faça seu login!"
        )
...
class LogarViewTest(TestCase):
    ...
    def test_logar_post_credenciais_corretas(self):
        ...
        response = self.client.post(
            self.logar_url,
            {"usuario": "testuser", "senha": "testpassword"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_logar_post_credenciais_incorretas_usuario(self):
        ...
        response = self.client.post(
            self.logar_url,
            {"usuario": "wronguser", "senha": "testpassword"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "logar.html")
        ...
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(
            str(messages_list[0]), "Usuario ou senha incorreto, tente novamente!"
        )
    ...
    def test_logar_post_credenciais_incorretas_senha(self):
        ...
        response = self.client.post(
            self.logar_url,
            {"usuario": "testuser", "senha": "wrongpassword"},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "logar.html")
    ...
    def test_logar_post_campos_vazios(self):
        ...
        response = self.client.post(
            self.logar_url, {"usuario": "", "senha": ""}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "logar.html")
        self.assertFalse("_auth_user_id" in self.client.session)
...
class SolicitarVendedorViewTest(TestCase):
    ...
    def test_solicitar_vendedor_post_nova_solicitacao(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "Solicitado com sucesso, aguarde até darmos uma resposta!",
    ...
    def test_solicitar_vendedor_post_solicitacao_existente(self):
        Solicitacao_Vendedor.objects.create(
            usuario=self.user, nome_completo="Existente", cpf="...", descricao="..."
        )
        self.client.login(username="testuser", password="testpassword")
        post_data = {
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "Você ja mandou solicitação! Aguarde a verificação!",
            [m.message for m in messages.get_messages(response.wsgi_request)],
        )
        self.assertEqual(Solicitacao_Vendedor.objects.count(), 1)
...
class VerSolicitacaoViewTest(TestCase):
    def setUp(self):
        ...
        self.solicitacao1 = Solicitacao_Vendedor.objects.create(
            usuario=self.regular_user,
            nome_completo="Solicitante 1",
            cpf="111",
            descricao="A",
        )
        self.ver_solicitacao_url = reverse("ver_solicitacao")
        self.aceitar_solicitacao_url = reverse(
            "aceitar_solicitacao", kwargs={"username": self.regular_user.username}
        )
        self.recusar_solicitacao_url = reverse(
            "recusar_solicitacao", kwargs={"username": self.regular_user.username}
        )
    ...
    def test_ver_solicitacao_get_admin_logado(self):
        ...
        self.assertEqual(
            response.context["solicitacoes"][0].usuario.username, "testuser"
        )
...
class AceitarSolicitacaoViewTest(TestCase):
    ...
    def test_aceitar_solicitacao_admin_logado(self):
        ...
        self.assertFalse(
            Solicitacao_Vendedor.objects.filter(usuario=self.regular_user).exists()
        )
...
class EditarPerfilViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            username="testuser", password="testpassword", first_name="Test"
        )
        try:
        ...
        self.other_user = User.objects.create_user(
            username=other_username, password="otherpassword"
        )
        try:
        ...
        self.editar_perfil_url = reverse(
            "editar_perfil", kwargs={"username": "testuser"}
        )
    ...
    def test_editar_perfil_post_salvar_nome_bios(self):
        ...
        post_data = {"salvar": "Salvar", "nome": "Novo Nome", "bios": "Nova biografia"}
    ...
    def test_editar_perfil_post_excluir_conta(self):
        ...
        response = self.client.post(
            self.editar_perfil_url, {"excluir": "Excluir"}, follow=True
        )
        self.assertRedirects(response, self.home_url)
...
class ListaProdutosViewTest(TestCase):
    def setUp(self):
        ...
        self.lista_produtos_url_self = reverse(
            "lista_produtos", kwargs={"username": "testuser"}
        )
        self.lista_produtos_url_other = reverse(
            "lista_produtos", kwargs={"username": "otheruser"}
        )
        self.home_url = reverse("home")
        self.client.force_login(self.user)
    ...
    def test_lista_produtos_get_usuario_inexistente(self):
        ...
        inexistente_url = reverse(
            "lista_produtos", kwargs={"username": "nonexistentuser"}
        )
...
class VendasViewTest(TestCase):
    def setUp(self):
        ...
        self.cliente = User.objects.create_user(
            username="cliente", password="password2"
        )
        ...
        self.order1 = Order.objects.create(
            vendedor=self.vendedor, comprador=self.cliente, valor_total_pedido=50.00
        )
        self.order2 = Order.objects.create(
            vendedor=self.vendedor, comprador=self.cliente, valor_total_pedido=100.00
        )
        self.order_outro_vendedor = Order.objects.create(
            vendedor=self.cliente, comprador=self.vendedor, valor_total_pedido=25.00
        )
    ...
    def test_vendas_get_cliente_logado(self):
        ...
        if "orders" in response.context:
            orders_vendedor_cliente = [
                order
                for order in response.context["orders"]
                if order.vendedor == self.cliente
            ]
